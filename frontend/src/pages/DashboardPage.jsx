...
export default function DashboardPage() {
  ...
  return (
    <div className="p-4 lg:p-7 min-h-full bg-[linear-gradient(180deg,#0a0f0d_0%,#090c0b_100%)]">
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
        {[
          ...
          <motion.div
            ...
            className="rounded-xl border border-border-medium bg-bg-card p-5"
          >
            <p className="text-[10px] uppercase tracking-widest text-text-secondary mb-2">{label}</p>
            <p className="font-heading font-bold text-4xl text-accent leading-none">{value}</p>
          </motion.div>
        ))}
      </div>

      <div className="rounded-xl border border-border-medium bg-bg-card overflow-hidden">
        <div className="flex items-center justify-between px-4 lg:px-6 py-4 border-b border-border-default">
          <h2 className="font-heading font-bold text-3xl">Recent Processed Streams</h2>
          <button
            onClick={() => navigate('/app/analyze')}
            className="flex items-center gap-2 px-4 py-2 rounded text-xs font-bold text-[#031a0b] bg-btn-primary"
          >
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-text-secondary text-[10px]">{job.status === 'processing' ? 'Processing...' : 'Match footage'}</p>
                  </div>
                </div>
      ...
      >
        <Plus size={20} className="text-[#031a0b]" />
      </button>
    </div>
  )
}
